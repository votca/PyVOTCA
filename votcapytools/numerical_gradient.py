...
__all__ = ["NumericalGradient", "read_xyz"]
...
def read_xyz(fin):
    ...
    return namedtuple("XYZFile", ["coords", "title", "atomtypes"])(coords, title, atomtypes)
...
def run_votca(atom, direction, name, case, threads):
    ...
    name_disp = "{}_at{}_dir{}_{}".format(
        str(name), str(atom), str(direction), str(case))
    votcacmd = "xtp_tools -e dftgwbse -o dftgwbse.xml -n {} -t {} > ./experiments/logfile_{}.log".format(
        str(name_disp), str(threads), str(name_disp))
    os.system(votcacmd)
    os.replace('{}.orb'.format(str(name_disp)),
               './experiments/{}.orb'.format(str(name_disp)))
