...
def test_QP_corrections():
    ...
    qp_corr = mol.get_qp_corrections()
...
def test_oscillatorStrengths():
    ...
    osc_ref = np.array([3.93892214e-02, 3.93892190e-02, 7.26550288e-10, 4.52526432e-16,
                        4.52526432e-16])
    e, osc = mol.get_oscillator_strengths()
    ...
    osc_dyn_ref = np.array([3.87883037e-02, 3.87882995e-02, 7.16099511e-10, 4.46337187e-16,
                            4.46337187e-16])
    e_dyn, osc_dyn = mol.get_oscillator_strengths(dynamic=True)
...
class ExceptionTests(unittest.TestCase):
    ...
    def test_for_data(self):
        mol = Molecule()
        self.assertRaises(Exception, mol.check_data)
        self.assertRaises(Exception, mol.get_dft_energy)
        self.assertRaises(Exception, mol.get_ks_total_energy, 1)
        self.assertRaises(Exception, mol.get_qp_corrections)
        self.assertRaises(Exception, mol.get_oscillator_strengths)

    def test_existing_molecule_coordinates(self):
        mol = Molecule()
        mol.add_atom("C", 0, 0, 0)
        mol.add_atom("O", 1.3, 0, 0)
        self.assertRaises(Exception, mol.read_orb, PATH_TEST / "example.orb")
